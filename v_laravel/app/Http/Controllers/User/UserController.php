...
class UserController extends Controller
{
    ...
    public function info()
    {
      ...
      $user = User::select('users.img','users.username','users.id as userid','users.email','users.country_id','users.theme_color','c.country_name','r.rank_name')
    ...
    public function store(Request $request)
    {
     ...
     // ->join('maincategorys', 'mc.subcategory_id', '=', 'sc.id')
      ...
      // ->orderBy('posts.updated_at', 'desc')     
       ...
       $update_info = DB::table('users')
       ->where('id', $user_id)
       ->update(['img' => $user->img ,'theme_color' =>  $request->theme_color,'rank_id' => $request->rank_id ]);
     }
     ...
     if (!empty($file)) {
      ...
      // ->update(['img' => $random_number_img ,'country_id' => $request->country_id,'theme_color' =>  $request->theme_color,'rank_id' => $request->rank_id ]);
       ...
       $update_info = DB::table('users')
      ->where('id', $user_id)
      ->update(['img' => $random_number_img ,'theme_color' =>  $request->theme_color,'rank_id' => $request->rank_id ]);
    }
